...
class ConnectorPluginView extends View
  ...
  # TODO: Improve GitHub issue integration.  Allow for checking of tasks to update issue for DoD. gh:286 id:69
  showRelatedIssues: () ->
    @relatedIssues.empty()
    ...
    async.map @issues, (number, cb) =>
      ...
      @client.getIssue @connector, number, (err, issue) =>
        cb(err, issue)
    , (err, results) =>
      # TODO: Check error for 404/Not Found when getting an issue from provider. +enhancement gh:203 id:78
      # TODO: Be sure to fire waffle rules on the same request as the github issue creation to ensure it starts off in the right waffle list +enhancement gh:204 id:93
      if err
  ...
  newIssue: ->
    # TODO: Also add the task list as a label when creating an issue on github. +waffle +feature +new id:67 gh:300 ic:gh
