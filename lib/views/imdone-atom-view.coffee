...
# BACKLOG:30 Add keen stats for features
module.exports =
class ImdoneAtomView extends ScrollView
  ...
  addPluginProjectButtons: -> @menuView.addPluginProjectButtons @plugins # DOING:210 Add the plugin project buttons here
  ...
  onRepoUpdate: ->
    # BACKLOG:160 This should be queued so two updates don't colide
    @showMask()
    @updateBoard()
  ...
  # BACKLOG:140 Split this apart into it's own class to simplify. Call it BoardView +refactor
  updateBoard: ->
    @destroySortables()
    @board.empty().hide()
    repo = @imdoneRepo
    ...
    # BACKLOG:190 We can display data from imdone.io in a card summary/details
    getTask = (task) =>
    ...
    @filter()
    @board.show()
    @hideMask() # TODO:190 hide mask on event from connectorManager who will retry after emitting
    @makeTasksSortable()
