...
class ImdoneAtomView extends ScrollView
  ...
  handleEvents: ->
    ...
    @connectorManager.on 'tasks.syncing', => @showMask() # READY:340 mask isn't always hiding correctly gh:105 id:69

    @connectorManager.on 'sync.error', => @hideMask()

    @connectorManager.on 'tasks.updated', => # READY:120 If syncing don't fire onRepoUpdate.  Wait until done syncing. gh:105 id:70
      @onRepoUpdate()
  ...
  addPluginProjectButtons: -> @menuView.addPluginProjectButtons @plugins # DOING:0 Add the plugin project buttons here id:72 githubClosed:true
  ...
  addPlugin: (Plugin) ->
    @connectorManager.getProduct Plugin.provider, (err, product) => # READY:110 Get the connector from the connector manager id:73
  ...
  updateBoard: ->
    ...
    @filter()
    @board.show()
    @hideMask() # TODO:220 hide mask on event from connectorManager who will retry after emitting githubClosed:true id:83
    @makeTasksSortable()
