...
class ProductDetailView extends View
  ...
  createEditor: ->
    options =
      schema: @product.schemas.config # TODO:160 Rule schemas to be set by GET /projects/ :projectId/products +rules-workflow id:97
      startval: @product.connector.config # TODO:170 Rule values to be set by GET /projects/ :projectId/products +rules id:98
      theme: 'bootstrap3'
    ...
    # TODO:40 Add provider configurations before creating editor id:99
  ...
  # READY:30 Add enable checkbox and take appropriate actions on check/uncheck +urgent id:100
  # READY:300 When unlinked disable all connectors (In API) +urgent id:101
  getDetail: (product) ->
    $$ ->
      @h1 "#{product.name}"
      # TODO:180 This will have to be upadted on an event sent with pusher id:102
      @div class:'block', =>
        if product.isLinked()
          @div class:'btn-group', =>
