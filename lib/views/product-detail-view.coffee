...
class ProductDetailView extends View
  ...
  handleChange: ->
    ...
    @imdoneRepo.saveConnector connector, (err, connector) =>
      # TODO: Handle errors by unauthenticating if needed and show login with error gh:116 id:28
      # BACKLOG: Need a way to handle repos that don't allow issues (Forks, etc).  Maybe two settings. (gitub repo and github issues repo) +enhancement gh:142 id:17
      return if err
