...
# DONE: implement socket server to handle opening files in configured client issue:48 id:17
module.exports =
  clients: {}
  init: (port) ->
    ...
    log       = require './log'
    # DONE: Check if something else is listening on port issue:51 id:18
  ...
  tryProxy: (port) ->
    # DONE: First check if it's imdone listening on the port issue:52 id:19
    # DONE: if imdone is listening we should connect as a client and use the server as a proxy issue:52 id:20
  ...
  openFile: (project, path, line, cb) ->
    ...
    # DONE: only send open request to editors who deserve them issue:48 id:22
