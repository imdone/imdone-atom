...
# DONE:0 implement socket server to handle opening files in configured client issue:48
module.exports =
  clients: {}
  init: (port) ->
  ...
  tryProxy: (port) ->
    ...
    # DONE:0 if imdone is listening we should connect as a client and use the server as a proxy issue:52
    # BACKLOG:260 if imdone is not listening we should ask for another port issue:52
    log 'Trying proxy'
  ...
  openFile: (project, path, line, cb) ->
    ...
    # DONE:0 only send open request to editors who deserve them issue:48
