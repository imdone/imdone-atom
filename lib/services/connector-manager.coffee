...
class ConnectorManager extends Emitter
  ...
  constructor: (@repo) ->
    ...
    @handleEvents()
    # READY:0 Check for updates to products/connectors and update @products with changes id:14

  handleEvents: ->
    # DONE:0 Listen for events on repo and update imdone.io with tasks, but on first run we'll have to queue them up for after auth +story id:15
