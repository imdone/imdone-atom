...
# READY:240 The client public_key, secret and pusherKey should be configurable id:25
PROJECT_ID_NOT_VALID_ERR = new Error "Project ID not valid"
NO_RESPONSE_ERR = new Error "No response from imdone.io"
baseUrl = config.baseUrl # READY:250 This should be set to localhost if process.env.IMDONE_ENV = /dev/i id:26
...
class ImdoneioClient extends Emitter
  ...
  # TODO:150 If we get a forbidden error, then emit auth failure id:27
  doGet: (path) ->
  ...
  authFromStorage: (cb) ->
    ...
    @loadCredentials (err) =>
      ...
      @_auth (err, user) =>
        ...
        # TODO:250 if err.status == 404 we should show an error id:28
        cb err, user
  ...
  onAuthFailure: (err, res, cb) ->
    # READY:30 Add imdone_status to the Error id:29
  ...
  handlePushEvents: () ->
    @pusher = new Pusher config.pusherKey,
      ...
      disableStats: true
    # READY:310 imdoneio pusher channel needs to be configurable id:30
  ...
  getProducts: (projectId, cb) ->
    # READY:170 Implement getProducts id:31
  ...
  getProject: (projectId, cb) ->
    # READY:180 Implement getProject id:32
  ...
  getIssue: (connector, number, cb) ->
    # TODO:200 We have to be better about communicating errors from connector api response such as insufficient permissions with github gh:116 id:33
  ...
  createConnector: (repo, connector, cb) ->
    ...
    # READY:110 Implement createProject id:34
  ...
  updateConnector: (repo, connector, cb) ->
    projectId = @getProjectId repo
    return cb "project must have a sync.id to connect" unless projectId
    # READY:120 Implement createProject id:35
  ...
  getOrCreateProject: (repo, cb) ->
    # READY:160 Implement getOrCreateProject id:39
  ...
  tasksDb: (repo) ->
    #READY:340 return the project specific task DB id:42
    @db 'tasks',repo.getPath().replace(/\//g, '_')

  listsDb: (repo) ->
    #READY:350 return the project specific task DB id:43
  ...
  #   # READY:150 Implement createTasks id:44
  #   # READY:330 modifyTask should update text with metadata that doesn't exists id:45
  ...
  #   # READY:190 Implement getProject id:46
  ...
  #   # READY:300 getOrCreateProject should happen when we get products, if we know a product is linked id:50
