...
# READY:260 The client public_key, secret and pusherKey should be configurable id:26
PROJECT_ID_NOT_VALID_ERR = new Error "Project ID not valid"
NO_RESPONSE_ERR = new Error "No response from imdone.io"
baseUrl = config.baseUrl # READY:270 This should be set to localhost if process.env.IMDONE_ENV = /dev/i id:27
...
class ImdoneioClient extends Emitter
  ...
  # TODO:140 If we get a forbidden error, then emit auth failure id:28
  doGet: (path) ->
  ...
  authFromStorage: (cb) ->
    ...
    @loadCredentials (err) =>
      ...
      @_auth (err, user) =>
        ...
        # TODO:230 if err.status == 404 we should show an error id:29
        cb err, user
  ...
  onAuthFailure: (err, res, cb) ->
    # READY:50 Add imdone_status to the Error id:30
  ...
  handlePushEvents: () ->
    @pusher = new Pusher config.pusherKey,
      ...
      disableStats: true
    # READY:330 imdoneio pusher channel needs to be configurable id:31
  ...
  getProducts: (projectId, cb) ->
    # READY:190 Implement getProducts id:32
  ...
  getProject: (projectId, cb) ->
    # READY:200 Implement getProject id:33
  ...
  getIssue: (connector, number, cb) ->
    # TODO:190 We have to be better about communicating errors from connector api response such as insufficient permissions with github gh:116 id:34
  ...
  createConnector: (repo, connector, cb) ->
    ...
    # READY:130 Implement createProject id:35
  ...
  updateConnector: (repo, connector, cb) ->
    projectId = @getProjectId repo
    return cb "project must have a sync.id to connect" unless projectId
    # READY:140 Implement createProject id:36
  ...
  createProject: (repo, cb) ->
    ...
    @doPost("/projects").send(
      ...
      return cb(err, res) if err || !res.ok
      project = res.body
      # BACKLOG:190 This should be in connectorManager id:39
      @setProjectId repo, project.id
      @setProjectName repo, project.name
      repo.saveConfig (err) => cb err, project

  getOrCreateProject: (repo, cb) ->
    # READY:180 Implement getOrCreateProject id:40
    # BACKLOG:270 move this to connectorManager id:41
    # DONE:0 Make sure this works github_closed:true id:42
  ...
  tasksDb: (repo) ->
    #READY:360 return the project specific task DB id:43
    @db 'tasks',repo.getPath().replace(/\//g, '_')

  listsDb: (repo) ->
    #READY:370 return the project specific task DB id:44
  ...
  #   # READY:170 Implement createTasks id:45
  #   # READY:350 modifyTask should update text with metadata that doesn't exists id:46
  ...
  #   # READY:210 Implement getProject id:47
  ...
  #   # BACKLOG:170 Should we really do this for all local tasks or do we ask api for task id's, dates and text checksum?  We can compare them before running rules. id:48
  ...
  #         # BACKLOG:230 Use rules to determine if and how cloud tasks and local tasks should be synced id:49
  #       cb()
  #
  # syncTasks: (repo, tasks, product, cb) ->
  #   cb = if cb then cb else () ->
  #   # BACKLOG:110 Emit progress through the repo so the right board is updated id:50
  #   # READY:320 getOrCreateProject should happen when we get products, if we know a product is linked id:51
