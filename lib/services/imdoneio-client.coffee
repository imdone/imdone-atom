...
# DONE: The client public_key, secret and pusherKey should be configurable id:24
...
baseUrl = config.baseUrl # DONE: This should be set to localhost if process.env.IMDONE_ENV = /dev/i id:25
...
class ImdoneioClient extends Emitter
  ...
  onAuthFailure: (err, res, cb) ->
    # DONE: Add imdone_status to the Error id:28
  ...
  handlePushEvents: () ->
    @pusher = new Pusher config.pusherKey,
      ...
      disableStats: true
    # DONE: imdoneio pusher channel needs to be configurable id:29
  ...
  getProducts: (projectId, cb) ->
    # DONE: Implement getProducts id:30
  ...
  getProject: (projectId, cb) ->
    # DONE: Implement getProject id:31
  ...
  createConnector: (repo, connector, cb) ->
    ...
    # DONE: Implement createProject id:33
  ...
  updateConnector: (repo, connector, cb) ->
    projectId = @getProjectId repo
    return cb "project must have a sync.id to connect" unless projectId
    # DONE: Implement createProject id:34
  ...
  # DONE: Send branch on sync if available for rules. gh:135 +now id:35
  ...
  tasksDb: (repo) ->
    #DONE: return the project specific task DB id:36
    @db 'tasks',repo.getPath().replace(/\//g, '_')

  listsDb: (repo) ->
    #DONE: return the project specific task DB id:37
