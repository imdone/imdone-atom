...
# READY:180 The client public_key, secret and pusherKey should be configurable
PROJECT_ID_NOT_VALID_ERR = new Error "Project ID not valid"
baseUrl = config.baseUrl # READY:170 This should be set to localhost if process.env.IMDONE_ENV = /dev/i
...
class ImdoneioClient extends Emitter
  ...
  handlePushEvents: () ->
    @pusher = new Pusher config.pusherKey,
      ...
      disableStats: true
    # READY:100 imdoneio pusher channel needs to be configurable
  ...
  getProducts: (projectId, cb) ->
    # READY:220 Implement getProducts
  ...
  getOrCreateProject: (repo, cb) ->
    # READY:110 Implement getOrCreateProject
  ...
  createTasks: (repo, project, tasks, product, cb) ->
    # READY:160 Implement createTasks
    # READY:70 modifyTask should update text with metadata that doesn't exists
  ...
  syncTasks: (repo, tasks, product, cb) ->
    ...
    # READY:40 getOrCreateProject should happen when we get products, if we know a product is linked
  ...
  tasksDb: (repo) ->
    #READY:90 return the project specific task DB
